...
public class Table
{
  ...
  { 
        ...
        else //winningSeats is at least 1
        {
          for (int j = winningSeats.size() - 1; j > -1; j--)
          {
            ...
            int winner = PokerUtil.getWinner(cat1, p.getShowDownHand(), 
                                             cat2, players[winningSeats.get(j)].getShowDownHand());//returns1ifhand1won
            boolean hasMoreOrEqualEquity = true; //false if p1 is all in for less then the pot
            if (p.getAllIn() != -1) //p is all in
            {
              if (players[winningSeats.get(j)] = -1 || p.getAllIn() < players[winningSeats.get(j)].getAllIn())
    ...
    for(int i = winningSeats.size(); i > -1; i--) 
    {
      for (int j = i - 1; j > 0; j --)
      {
        ...
        {
          players[winningSeats.get(i)].addChips(players[winningSeats.get(i)].getAllIn()); //difference between i and j is the number of hands that the getAllIn needs to be splt with
  ...
  public int getNumActivePlayers()
  {
    ...
    for (Player p : players)
    {
      if(!p.hasFolded() && p.allIn =! -1)// not folded and not all in
        left++;
    }
    return left;
  }
  ...
  public void bet(boolean preflop)
  {
    ...
    while(called < playersLeft && playersLeft > 1)
    {
      ...
      if (!activePlayer.hasFolded() && activePlayer.getAllIn() == -1) //player is not yet all in
      {
        ...
        else if(option > (call - activePlayer.getBet())) //Player Raised
        {
          ...
          {
            if (!player[i].isfolded && player[i].getStack() == 0 && player[i].getBet() == call && call > 0)
            {
              playersLeft--;
            }
          }
    ...
    for (int i = 0; i < players.length; i++)
    {
      ...
      if (p.getBet() > 0 && p.getBet() < call ) //player was all in and placed a bet this round
      {
        ...
        {
          if (!players[j].isFolded() && players[j].getBet() >= p.getBet())
